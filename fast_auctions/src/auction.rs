...
fn max_cost_upper_bound(x: &ArrayView2<f64>, y: &ArrayView2<f64>) -> f64 {
    ...
    //To obtain this bound, we just compute the bounding rectangle of x and y
    //that touches the diagonal
    ...
    for point in x.outer_iter().chain(y.outer_iter()) {
...
pub fn auction_algorithm(
    ...
    off_diag_prices: Option<&mut ArrayViewMut1<f64>>,
    diag_prices: Option<&mut ArrayViewMut1<f64>>,
    delta: f64,
) -> f64 {
    ...
    //Write input prices if some are given.
    if let Some(off_diag_prices) = off_diag_prices.as_ref() {
    ...
    if let Some(diag_prices) = diag_prices.as_ref() {
    ...
    while d2 > (1.0 + delta) * (d2 - n * epsilon) {
        ...
        //Reset the matching
        unassigned_off_diag.clear();
        unassigned_diag.clear();
        unassigned_off_diag.extend(0..(x_size as u32));
        unassigned_diag.extend(0..(y_size as u32));

        reverse_off_diag.fill(NO_BIDDER);
        reverse_diag.fill(NO_BIDDER);
        ...
        'round: loop {
            ...
            if let Some(&off_diag_bidder) = unassigned_off_diag.iter().next() {
                ...
                let diag_cost = x_diag_distances[off_diag_bidder as usize] + diag_objects.get_price(off_diag_bidder);
            ...
            if let Some(&diag_bidder) = unassigned_diag.iter().next() {
                ...
                let off_diag_cost = y_diag_distances[diag_bidder as usize] + off_diag_objects.get_price(diag_bidder);
        ...
        //Recompute d2
        d2 = d2_from_matching(x, y, &off_diag.view(), &diag.view(), &x_diag_distances, &y_diag_distances);
    }

    //Write back prices if necessary
     if let Some(off_diag_prices) = off_diag_prices {
        for (i, price) in off_diag_prices.iter_mut().enumerate() {
            *price = off_diag_objects.get_price(i as u32);
        }
    }
    if let Some(diag_prices) = diag_prices {
